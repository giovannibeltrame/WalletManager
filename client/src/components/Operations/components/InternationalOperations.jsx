...
class InternationalOperations extends React.Component {
	constructor(props) {
		super(props);
	...
	renderTotalRescued = (totalRescued) => {
		return (
			<div>
				...
				<span>{numberToDollars(totalRescued)}</span>
			</div>
		);
	};
	...
	renderTotalYield = (totalYield) => {
		return (
			<div>
				...
				<span>{numberToDollars(totalYield)}</span>
			</div>
		);
	};
	...
	renderTotalCosts = (totalCosts) => {
		return (
			<div>
				...
				<span>{numberToDollars(totalCosts)}</span>
			</div>
		);
	};
	...
	renderAveragePrice = (averagePrice) => {
		return (
			<div>
				...
				<span>{numberToDollars(averagePrice)}</span>
			</div>
		);
	};
	...
	renderGrossBalance = (grossBalance) => {
		return (
			<div>
				...
				<span>{numberToDollars(grossBalance)}</span>
			</div>
		);
	};
	...
	renderResult = (result) => {
		...
		return (
			<div>
				...
				<span>{numberToDollars(result)}</span>
			</div>
		);
	};
	...
	getTableColumns() {
		return [
			...
			},
			{
				...
				text: 'Preço Unitário',
				formatter: (cell, row, rowIndex, formatExtraData) =>
					row.unitPrice ? numberToDollars(row.unitPrice) : '-',
			},
			{
				dataField: 'amount',
				align: 'right',
					...
					<Col>
						<div>
							<small>
								...
								{numberToDollars(row.settlementFee)}
							</small>
						</div>
						<div>
							<small>
								<b>Emolumentos: </b>
								{numberToDollars(row.emoluments)}
							</small>
						</div>
						<div>
							<small>
								<b>Impostos: </b>
								{numberToDollars(row.taxes)}
							</small>
						</div>
						<div>
							<small>
								<b>Corretagem: </b>
								{numberToDollars(row.brokerage)}
							</small>
						</div>
						<div>
							<small>
								<b>Total em custos: </b>
								{numberToDollars(row.costs)}
							</small>
						</div>
					</Col>
					<Col>
	...
	render() {
		...
		if (asset.description === 'Caixa USD')
			return <CurrencyOperations asset={asset} />;
			...
			<Container>
				...
				{asset.ticker && asset.lastUnitPrice ? (
					<h5 className='text-secondary'>
						{asset.ticker} - {numberToDollars(asset.lastUnitPrice)}
					</h5>
				) : null}
